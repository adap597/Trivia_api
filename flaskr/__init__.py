  ...
  def get_quizzes():
    try:
      ...
      if category_id == 0:
        ...
        quiz_questions = Question.query.filter_by(category=category_id).filter(Question.id.notin_((previous_questions))).all()

      if quiz_questions:
        #See https://docs.python.org/3/library/random.html
        quiz_questions = quiz_questions[random.randrange(0, len(quiz_questions)-1)]

      return jsonify({
        'success': True,
        'question': quiz_questions.format()
      })
    except:
      abort(422)
